...
``_build_stage_timeout`` returns ``5 + 8 * wp_count`` minutes, capped at
90. The helper reads ``build_order`` first (the supervisor populates it)
...
def test_build_stage_timeout_single_wp() -> None:
    # 5 base + 8 per WP × 1 = 13 minutes.
    assert _build_stage_timeout({"build_order": ["wp-1"]}) == timedelta(minutes=13)


def test_build_stage_timeout_three_wp_from_build_order() -> None:
    state = {"build_order": ["wp-1", "wp-2", "wp-3"]}
    # 5 + 8*3 = 29 minutes.
    assert _build_stage_timeout(state) == timedelta(minutes=29)
